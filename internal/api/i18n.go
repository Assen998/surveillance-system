...
var envMsgs = map[string]map[string]string{
	localeZH: {
		...
		"env.hw.encode":        "编码",
		"env.hw.decode":        "解码",
		"env.hw.found":         "可用硬件编解码：%s（录像走 copy 透传不受影响；预览转码当前使用 CPU 软件编码）",
	...
	localeEN: {
		...
		"env.hw.found":         "Available HW codecs: %s (recording uses copy passthrough; preview transcode currently uses CPU software encoding)",
